...
app.post('/regisDB', async (req,res) => {
  let sql = "CREATE TABLE IF NOT EXISTS userinfo (username VARCHAR(200),password VARCHAR(200),getlike INT,getscore INT,comment JSON)";
  let result = await queryDB(sql);

  const newUser = req.body;
  const username = newUser.username;
  const password = newUser.password[0];
  let comment = ['Hello!', 'You can comment your score here']
